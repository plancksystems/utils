...
pub fn createServiceArchive(allocator: Allocator, io: Io, output_path: []const u8, vlog_dir: []const u8, index_dir: []const u8, secondary_index_paths: []const []const u8, wal_dir: []const u8, now_ms: i64) !format.BackupMetadata {
    ...
    std.mem.sort([]const u8, vlog_files.items, {}, struct {
    ...
    for (vlog_files.items) |vlog_path| {
        ...
        const prev = pos;
        const r = try writeFileSection(allocator, io, backup_file, vlog_path, rel, .ValueLog, pos);
        total_entries += r.entries;
        pos = r.pos;
        if (pos != prev) actual_count += 1;
    }

    {
        ...
        const prev = pos;
        const r = try writeFileSection(allocator, io, backup_file, primary_src, "indexes/primary.idx", .Index, pos);
        pos = r.pos;
        if (pos != prev) actual_count += 1;
    }

    {
        ...
        const prev = pos;
        const r = try writeFileSection(allocator, io, backup_file, catalog_src, "indexes/system.catalog.idx", .Index, pos);
        pos = r.pos;
        if (pos != prev) actual_count += 1;
    }
    ...
    for (secondary_index_paths) |src_path| {
        ...
        defer allocator.free(rel);
        const prev = pos;
        const r = try writeFileSection(allocator, io, backup_file, src_path, rel, .Index, pos);
        pos = r.pos;
        if (pos != prev) actual_count += 1;
    }
    ...
    for (wal_files.items) |wal_path| {
        ...
        defer allocator.free(rel);
        const prev = pos;
        const r = try writeFileSection(allocator, io, backup_file, wal_path, rel, .Wal, pos);
        pos = r.pos;
        if (pos != prev) actual_count += 1;
    }
