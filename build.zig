...
pub fn build(b: *std.Build) void {
    ...
    // ── docs ──
    // Generates a synthetic `docs_root.zig` that imports every `.zig` file
    // under `src/` so autodoc covers the entire tree, not just what
    // `root.zig` reaches. Stages the full src/ so relative `@import`
    // and `@embedFile` keep resolving.
...
// ── docs root generator ────────────────────────────────────────────────
//
// Walks `<src_dir>/` on the host at build-configure time and emits a
// `docs_root.zig` that imports every `.zig` file as a pub const. Stages
// the full subtree (zig + non-zig) into the same WriteFile group so
// relative imports and `@embedFile` paths resolve from the staged copy.
